...
float *K210_GetIPD(unsigned short timeOut)
{
	...
	do
	{
        ...
        if (token != NULL) 
        {
            location_xyz[1] = (160-atof(token))/40*2.9;//像素坐标转换，并对世界坐标的y赋值
            token = strtok_r(NULL, delimiters, &next_token);
            if (token != NULL) 
            {
                location_xyz[0] = (160-atof(token))/40*3.0 + 24 - 0.15 * __fabs(location_xyz[1]);//对世界坐标的x赋值,24是转换的空间变换长度（cm）,0.15是边缘修正长度（cm）
                K210_Clear();
