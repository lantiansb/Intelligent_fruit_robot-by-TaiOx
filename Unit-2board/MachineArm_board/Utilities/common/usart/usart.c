...
unsigned char cmd;//视觉抓取板获取小车板的抓取命令
...
//    GPIO_PullResistorConfig(HT_GPIOB, USART0_RX_PIN, GPIO_PR_UP);
    GPIO_PullResistorConfig(HT_GPIOA, USART1_RX_PIN, GPIO_PR_UP);
    GPIO_PullResistorConfig(HT_GPIOB, UART0_RX_PIN, GPIO_PR_UP);
...
//    AFIO_GPxConfig(USART0_GPIO_GROUP, USART0_RX_PIN, USART_AFIO_MODE);
    ...
    AFIO_GPxConfig(UART0_GPIO_GROUP, UART0_TX_PIN, USART_AFIO_MODE);
    AFIO_GPxConfig(UART0_GPIO_GROUP, UART0_RX_PIN, USART_AFIO_MODE);
  /*
		bound：     115200
		数据长度：   8bits
        停止位：    1位
