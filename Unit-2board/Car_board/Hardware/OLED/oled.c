...
//space 每一遍显示的间隔
//mode:0,反色显示;1,正常显示
u8 OLED_ScrollDisplay(u8 num,u8 space,u8 mode)
{
	u8 i,n,t=0,m=0,r;
	while(1)
	{
...
void OLED_Init(void)
{
    ...
    PeripCLKConfig_TypeDef.Bit.AFIO = 1;
	CKCU_PeripClockConfig(PeripCLKConfig_TypeDef, ENABLE);
    AFIO_GPxConfig(GPIO_PB, GPIO_PIN_3, AFIO_MODE_DEFAULT);
    AFIO_GPxConfig(GPIO_PB, GPIO_PIN_3, AFIO_MODE_DEFAULT);
    GPIO_DirectionConfig(HT_GPIOB, GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_7, GPIO_DIR_OUT);
    GPIO_SetOutBits(HT_GPIOB, GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_7);
	OLED_RES_Clr();
	delay_ms(200);
	OLED_RES_Set();
