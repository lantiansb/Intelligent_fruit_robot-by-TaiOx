...
void ESP8266_Init(void)
{
    ...
    GPIO_DirectionConfig(HT_GPIOB, GPIO_PIN_6, GPIO_DIR_OUT);            //选择为输出模式
    GPIO_PullResistorConfig(HT_GPIOB, GPIO_PIN_6, GPIO_PR_DISABLE);      //上拉电阻
    GPIO_DriveConfig(HT_GPIOB, GPIO_PIN_6, GPIO_DV_8MA);                 //选择指定GPIO-PIN的驱动电流，可选GPIO_DV_4/8/12/16MA
	GPIO_ClearOutBits(HT_GPIOB, GPIO_PIN_6);//软件复位
	delay_ms(250);
...
void USART0_IRQHandler(void)
{
	...
	if(USART_GetFlagStatus(HT_USART0, USART_FLAG_RXDR) != RESET) //接收中断
	{
		if(esp8266_cnt >= sizeof(esp8266_buf))	esp8266_cnt = 0; //防止串口被刷爆
		esp8266_buf[esp8266_cnt++] = HT_USART0->DR;
		
		USART_ClearFlag(HT_USART0, USART_FLAG_RSADD);
	}
